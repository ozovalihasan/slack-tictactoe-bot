...
class Bot
  ...
  def self.start_game(user_id, msg, url)
    ...
    msg = {
      ...
      attachments: self.board_last(user_id)
    }
  ...
  def self.update_board(request_data,chosen,symbol)
    ...
    self.increase_turn_number(user_id)
  end
  ...
  def self.choose_position(user_id)
    available_positions = self.check_positions(user_id)
    return available_positions.uniq.select {|item| available_positions.count(item)==1}[0] if available_positions.class == Array
    $plays[user_id][:positions].select {|item| item.class == Integer }.sample
  end

  def self.check_winner_draw(user_id,url,msg)
    winner = self.check_positions(user_id)
  ...
  def self.check_positions(user_id)
    ...
    # check the rows
    return self.check_rows(grid) unless self.check_rows(grid).nil?

    # check the columns
    return self.check_columns(grid) unless self.check_columns(grid).nil?

    # check the diagonals
    return self.check_diagonals(grid) unless self.check_diagonals(grid).nil?

    nil
  end

  def self.check_rows(grid)
    grid.each { |row| return row.first if self.all_equal?(row) }
    grid.each { |row| return row if (self.any_equal?(row) && row.any? {|item| item.class == Integer})}
  ...
  def self.check_columns(grid)
    self.check_rows(grid.transpose)
  end
